...
class Logger:
    def __init__(self):
        try:
            # 从上下文中获取配置文件中的日志路径配置
            self._log_file = app.config.get('LOGGER_PATH') + '/' + "fuxi_{}.log".format(
                time.strftime("%Y_%m_%d", time.localtime())
            )
            if not os.path.exists(app.config.get('LOGGER_PATH')):
                os.makedirs(app.config.get('LOGGER_PATH'))
            self._logger = logging.getLogger("fuxi")
            self._logger.setLevel(logging.DEBUG)
            # 日志保存到日志目录
            file_handler = logging.FileHandler(self._log_file)
            self._logger.addHandler(hdlr=file_handler)
            # 日志打印 (标准输出) 只有配置文件打开 debug 才会打印在终端
