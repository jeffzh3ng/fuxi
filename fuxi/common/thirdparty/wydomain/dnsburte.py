...
# def run(args):
def run(domain, dictfile=None, outfile=None, threads=16, timeout=10):
    # domain = args.domain
    ...
    timeout = int(timeout)
    if not dictfile:
        dict_file = os.path.dirname(__file__) + "/wydomain.csv"
    else:
        dict_file = dictfile
    if not outfile:
        outfile = tempfile.gettempdir() + "/temp_subdomain.txt"
