...
# external modules
from fuxi.common.thirdparty.sublist3r.subbrute import subbrute
import dns.resolver
import requests
...
class enumratorBaseThreaded(Process, enumratorBase):
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if enable_bruteforce:
        ...
        # add system path ,jeffzhang +++++
        subs = os.path.join(path_to_file, 'subbrute', os.path.dirname(__file__) + "/subbrute/names.txt")
        # add system path ,jeffzhang +++++
        resolvers = os.path.join(path_to_file, 'subbrute', os.path.dirname(__file__) + "/subbrute/resolvers.txt")
